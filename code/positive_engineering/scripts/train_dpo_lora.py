...
def main():
    parser = argparse.ArgumentParser(
        description="Train a version-aware DPO LoRA adapter from replacement-vs-deprecated preference pairs."
    )
    ...
    parser.add_argument("--max-train-samples", type=int, default=0, help="0 means use all DPO pairs")
    parser.add_argument("--max-val-samples", type=int, default=0, help="0 means use all DPO pairs")
    parser.add_argument("--beta", type=float, default=0.1)
    parser.add_argument("--logprob-reduction", choices=["sum", "mean"], default="sum")
    parser.add_argument(
        "--dpo-scope",
        choices=["full", "api_span"],
        default="full",
        help="Use full completions or only replacement/deprecated API spans for DPO preference loss.",
    )
    parser.add_argument(
        "--api-anchor-weight",
        type=float,
        default=0.0,
        help="Weight for replacement API-token CE anchor. 0 keeps pure DPO behavior.",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional list of layer indices to enable for LoRA, e.g. '17,18,19' '20'.",
    )
    parser.add_argument("--gradient-checkpointing", action="store_true")
    parser.add_argument(
        "--keep-dropout",
        action="store_true",
        help="Keep dropout enabled. Default disables dropout for deterministic DPO policy/reference comparisons.",
    )
    parser.add_argument("--seed", type=int, default=1234)
    parser.add_argument(
        "--prompt-field",
        default="version_prompt",
        help="Prompt field to use from each data row. Use 'probing_input' to train without version context.",
    )
    ...
    layers_to_transform = parse_int_list(args.layers_to_transform) if args.layers_to_transform else None
    ...
    train_dataset = VersionAwareDPODataset(train_rows, tokenizer, args.max_length, prompt_field=args.prompt_field)
    val_dataset = VersionAwareDPODataset(val_rows, tokenizer, args.max_length, prompt_field=args.prompt_field)
        ...
        json.dump(
            {
                ...
                "reference_policy": "same base model with LoRA adapter disabled",
            },
            f,
            indent=2,
            ensure_ascii=False,
