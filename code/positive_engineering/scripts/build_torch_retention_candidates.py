...
CALL_RE = re.compile(r"torch\.([A-Za-z_][A-Za-z0-9_]*)\s*\(")
...
def collect_training_prefixes(dataset_paths: Iterable[Path]) -> List[str]:
    prefixes = set()
            ...
            for key in ("replacement_api", "deprecated_api"):
                ...
                for api_name in values:
                    ...
                    if not api_name.startswith("torch."):
                        continue
                    prefix = api_name[len("torch.") :].split(".", 1)[0].strip()
                    if prefix:
                        prefixes.add(prefix)
    ...
    *,
    ...
    for function_name, node in iter_function_nodes(tree):
        ...
        for match in CALL_RE.finditer(function_text):
            ...
            if not target_api or not target_api[0].islower():
                continue
            if target_api in exclude_prefixes:
                continue
            prompt = function_text[: match.start() + len("torch.")]
            suffix = function_text[match.start() + len("torch.") :]
            rows.append(
                {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build function-level torch API retention candidates from public code repos."
    )
    ...
    parser.add_argument("--min-api-count", type=int, default=5)
    parser.add_argument("--max-per-api", type=int, default=0, help="0 means keep all")
    args = parser.parse_args()

    exclude_prefixes = set(collect_training_prefixes(args.training_jsonl))
    raw_rows: List[Dict] = []
    ...
    api_counts = Counter(row["target_api"] for row in raw_rows)
    kept_rows = [row for row in raw_rows if api_counts[row["target_api"]] >= args.min_api_count]
    kept_rows.sort(
        key=lambda row: (
            row["target_api"],
            row["source_repo"],
    ...
    for row in kept_rows:
        key = (row["source_file"], row["function_name"], row["target_api"], row["source_char_offset"])
        if key in seen:
            continue
        if args.max_per_api > 0 and per_api_counter[row["target_api"]] >= args.max_per_api:
            continue
        seen.add(key)
        per_api_counter[row["target_api"]] += 1
        row = dict(row)
        row["id"] = f"torch_retention_candidate_{len(final_rows):05d}"
    ...
    summary = {
        ...
        "unique_apis": len(set(row["target_api"] for row in final_rows)),
        "top_apis": Counter(row["target_api"] for row in final_rows).most_common(80),
        "excluded_training_prefixes": sorted(exclude_prefixes),
    }
